...
- 使用 MinerU 作为主要 PDF 内容解析引擎。
- 使用 PyMuPDF 和 pypdf 作为 PDF 结构补充解析器。
...
2. 解析层：MinerU 主内容解析，以及 PyMuPDF/pypdf 补充解析。
...
- 合并 MinerU、PyMuPDF 和 pypdf 的输出。
- 对重复的图片、表单和元数据做去重。
- 将解析器特有数据转换为稳定的内部 Schema。
- 为关键内容单元保留来源信息。
- 对局部失败生成 warning，而不是静默丢弃。

### 5.8 Schema Extractor 字段抽取器
...
| 原生表单 | pypdf | PyMuPDF | 原生 PDF 字段值优先于文本推断的键值对。 |
| 元数据 | pypdf | PyMuPDF | 合并 key，冲突信息写入 diagnostics。 |
| 附件 | pypdf | PyMuPDF 兜底 | 按文件名和 sha256 去重。 |
| 批注 | PyMuPDF | pypdf | 作为可选辅助内容保留。 |
...
1. PDF 上传与任务创建。
2. 基于 pypdf/PyMuPDF 的 PDF 预检。
3. 基于 MinerU 的 PDF 解析。
4. 表单、元数据、附件和图片的补充抽取。
5. 统一解析 JSON。
...
- MinerU 执行方式：由解析 Worker 调用隔离的 Worker 进程，避免 API 进程被模型或运行时问题拖垮。
- 加速目标：优先支持 CPU 部署；有 GPU 环境时通过配置启用 GPU Worker。
...
一期使用 MinerU 作为主要 PDF 内容解析器，保留 PyMuPDF/pypdf 作为 PDF 结构补充解析器。不要把任何解析器的原始输出直接暴露为业务 API，而是先统一归一到稳定的内部 PDF JSON 模型，再在其上执行基于 Schema 的业务字段抽取。
